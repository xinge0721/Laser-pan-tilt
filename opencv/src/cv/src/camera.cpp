 ...
 * @brief 裁剪图像的中心区域
 * @param src 原始图像
 * @param crop_ratio 裁剪比例（0-1之间）
 * @return 裁剪后的图像
 */
cv::Mat cropCenterRegion(const cv::Mat& src, double crop_ratio = 0.5)
{
    // 获取图像尺寸
    int width = src.cols;
    int height = src.rows;
    // 计算中心区域边界
    int crop_width = static_cast<int>(width * crop_ratio);
...
int main(int argc, char *argv[])
{
    ...
    // 使用绿色HSV预设
    HsvThreshold hsvThresh = GREEN_HSV;
    
    // 创建HSV阈值调节滑块
    createHsvSliders("绿色HSV阈值", hsvThresh);
    ...
    while(1)
    {
        ...
        cv::erode(result, result, kernel, cv::Point(-1, -1), 7);        
        cv::imshow("形态", result);
